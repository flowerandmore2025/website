      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        {blog.posts.filter(post => post.title.includes('🌷')).length > 0 && (
          <div className="mx-auto mt-16 max-w-7xl">
            {blog.posts
              .filter(post => post.title.includes('🌷'))
              .map(post => (
                <article
                  key={post.slug}
                  ...
                  <div className="w-full md:w-2/3">
                    <div className="flex items-center gap-x-4 text-xs mb-4">
                      ...
                      <span className="relative z-1 rounded-full bg-primary-100 px-3 py-1.5 font-medium text-primary-600 hover:bg-primary-200">
                        {blog.categories.find(cat => cat.id === post.category)?.nameInThai ||
                          post.category}
                      </span>
                    </div>
                    <div className="group relative">
                      <h3 className="text-2xl font-bold leading-tight text-gray-900 group-hover:text-primary-600 font-display mb-4">
                        <Link
                          href={`/blog/${post.slug}`}
                          className="hover:underline decoration-primary-300"
                        >
                          {post.title}
                        </Link>
                      </h3>
                      <p className="text-base leading-relaxed text-gray-600 mb-6">{post.excerpt}</p>
                      <Link
                        href={`/blog/${post.slug}`}
